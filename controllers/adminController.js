 ...
 * Includes: total revenue, total rides, active riders, active drivers,
 * monthly performance, top drivers, top riders
 */
exports.getAnalyticsDashboard = async (req, res) => {
  try {
    ...
    const [
      ...
      // 1. Total Revenue (from completed rides)
      Ride.aggregate([
      ...
      // 3. Active Riders (online status)
      Rider.countDocuments({ status: "online", activeStatus: "active" }),
      
      // 4. Active Drivers (online or busy status)
      Driver.countDocuments({ 
        status: { $in: ["online", "busy"] }, 
        activeStatus: "active" 
      }),
      
      // 5. Monthly Performance Analysis (current year)
      Ride.aggregate([
        {
          $match: {
            createdAt: {
      ...
      // 6. Top 10 Performing Drivers (by completed rides)
      Ride.aggregate([
      ...
      // 7. Top 10 Riders by Activity (by total rides)
      Ride.aggregate([
        { $match: { rider: { $ne: null } } },
        {
          $group: {
    ...
    // Format monthly performance with month names
    const monthNames = [
      "January", "February", "March", "April", "May", "June",
      "July", "August", "September", "October", "November", "December"
    ];
    
    const formattedMonthlyPerformance = monthNames.map((monthName, index) => {
      const monthData = monthlyPerformance.find(m => m._id === index + 1);
      return {
        month: monthName,
        monthNumber: index + 1,
        totalRides: monthData?.totalRides || 0,
        completedRides: monthData?.completedRides || 0,
        cancelledRides: monthData?.cancelledRides || 0,
        revenue: monthData?.revenue || 0,
        platformCommission: monthData?.platformCommission || 0
      };
    });
    ...
    const analyticsData = {
      summary: {
        ...
        activeRiders,
        activeDrivers,
        completionRate: totalRides > 0 ? parseFloat(((completedRides / totalRides) * 100).toFixed(2)) : 0
      },
      monthlyPerformance: formattedMonthlyPerformance,
      topPerformingDrivers: topDrivers,
      topRidersByActivity: topRiders,
      generatedAt: new Date()
    };

    sendSuccess(res, analyticsData, "Analytics dashboard retrieved successfully", 200);
  } catch (err) {
    console.error("Get analytics dashboard error:", err);
    sendError(res, "Failed to retrieve analytics dashboard", 500);
  }
};
...
exports.getRevenueAnalytics = async (req, res) => {
  try {
    ...
    let dateMatch = { status: "completed" };
    ...
    switch (groupBy) {
      case "month":
        ...
        break;
      case "week":
        groupByField = { year: { $year: "$createdAt" }, week: { $week: "$createdAt" } };
        break;
      case "year":
    ...
    const totals = revenueData.reduce((acc, curr) => ({
      totalRevenue: acc.totalRevenue + curr.totalRevenue,
      platformCommission: acc.platformCommission + curr.platformCommission,
      driverEarnings: acc.driverEarnings + curr.driverEarnings,
      tips: acc.tips + curr.tips,
      ridesCount: acc.ridesCount + curr.ridesCount
    }), { totalRevenue: 0, platformCommission: 0, driverEarnings: 0, tips: 0, ridesCount: 0 });
...
exports.getRideStatistics = async (req, res) => {
  try {
    ...
    const [statusBreakdown, vehicleTypeBreakdown, paymentMethodBreakdown, hourlyDistribution] = await Promise.all([
      Ride.aggregate([
      ...
      Ride.aggregate([
        { $match: { status: "completed", ...(dateMatch.createdAt ? dateMatch : {}) } },
        { $group: { 
          _id: "$vehicleType", 
          count: { $sum: 1 },
          revenue: { $sum: "$fare" }
    ...
    const avgMetrics = await Ride.aggregate([
      { $match: { status: "completed", ...(dateMatch.createdAt ? dateMatch : {}) } },
      {
        $group: {
          _id: null,
          ...
          avgRiderRating: { $avg: "$rating.riderRating" },
          totalFare: { $sum: "$fare" },
          totalRides: { $sum: 1 }
        }
      }
    ...
    sendSuccess(res, {
      ...
      generatedAt: new Date()
    }, "Ride statistics retrieved successfully", 200);
  } catch (err) {
    console.error("Get ride statistics error:", err);
    sendError(res, "Failed to retrieve ride statistics", 500);
  }
};

/**
 * Get top performing drivers with detailed stats
 */
exports.getTopDrivers = async (req, res) => {
  try {
    const { limit = 10, sortBy = "rides", startDate, endDate } = req.query;
    
    // Build date match for rides (if date filters provided)
    let rideDateMatch = {};
    if (startDate || endDate) {
      rideDateMatch.createdAt = {};
      if (startDate) rideDateMatch.createdAt.$gte = new Date(startDate);
      if (endDate) rideDateMatch.createdAt.$lte = new Date(endDate);
    }
    ...
    switch (sortBy) {
      case "revenue":
        sortField = { totalRevenue: -1, totalRides: -1 };
        break;
      case "rating":
        sortField = { driverRating: -1, totalRides: -1 };
        break;
      case "earnings":
        sortField = { totalEarnings: -1, totalRides: -1 };
        break;
      default:
        sortField = { totalRides: -1, totalRevenue: -1 };
    }

    // Start from Driver collection to include ALL drivers (even those without rides)
    const topDrivers = await Driver.aggregate([
      // Match only approved/active drivers
      {
        $match: {
          isApproved: "approved",
          activeStatus: "active"
      ...
      // Lookup all rides for this driver
      {
        $lookup: {
          from: "rides",
          let: { driverId: "$_id" },
              ...
              $map: {
                input: {
                  $filter: {
                    input: "$rides",
                    as: "ride",
                    cond: { $ne: ["$$ride.rating.driverRating", null] }
                  }
                },
                as: "r",
                in: "$$r.rating.driverRating"
              }
            }
          }
        }
      ...
      // Project final output
      {
        $project: {
          _id: 1,
          driverName: { $ifNull: ["$userInfo.fullName", "Unknown"] },
          driverEmail: { $ifNull: ["$userInfo.email", "N/A"] },
          driverPhone: { $ifNull: ["$userInfo.phone", "N/A"] },
          totalRides: 1,
          completedRides: 1,
          cancelledRides: 1,
          completionRate: {
            $cond: {
              if: { $gt: ["$totalRides", 0] },
              then: { $round: [{ $multiply: [{ $divide: ["$completedRides", "$totalRides"] }, 100] }, 1] },
              else: 0
            }
          },
          totalRevenue: { $round: [{ $ifNull: ["$totalRevenue", 0] }, 2] },
          totalEarnings: { $round: [{ $ifNull: ["$totalEarnings", 0] }, 2] },
          totalTips: { $round: [{ $ifNull: ["$totalTips", 0] }, 2] },
          avgRideRating: { $round: [{ $ifNull: ["$avgRideRating", 0] }, 2] },
          driverRating: { $round: [{ $ifNull: ["$rating", 5] }, 2] },
          vehicleType: { $ifNull: ["$vehicleType", "N/A"] },
          vehicle: { $ifNull: ["$vehicle", "N/A"] },
          vehicleModel: { $ifNull: ["$vehicleModel", "N/A"] },
          vehicleYear: { $ifNull: ["$vehicleYear", null] },
          driverStatus: "$status",
          activeStatus: 1,
          isApproved: 1,
          joinedAt: "$createdAt"
    ...
    sendSuccess(res, {
      ...
      drivers: topDrivers,
      generatedAt: new Date()
    }, "Top performing drivers retrieved successfully", 200);
  } catch (err) {
...
/**
 * Get top riders by activity with detailed stats
 */
exports.getTopRiders = async (req, res) => {
  try {
    const { limit = 10, sortBy = "rides", startDate, endDate } = req.query;
    
    let dateMatch = { rider: { $ne: null } };
    if (startDate || endDate) {
      dateMatch.createdAt = {};
      if (startDate) dateMatch.createdAt.$gte = new Date(startDate);
      if (endDate) dateMatch.createdAt.$lte = new Date(endDate);
    ...
    const topRiders = await Ride.aggregate([
      ...
      { $unwind: { path: "$riderInfo", preserveNullAndEmptyArrays: true } },
      {
        $project: {
          ...
          totalTips: { $round: ["$totalTips", 2] },
          avgRating: { $round: [{ $ifNull: ["$avgRating", 0] }, 2] },
          avgFare: { $round: [{ $ifNull: ["$avgFare", 0] }, 2] },
          completionRate: {
            $cond: [
              { $eq: ["$totalRides", 0] },
              0,
              { $round: [{ $multiply: [{ $divide: ["$completedRides", "$totalRides"] }, 100] }, 2] }
            ]
          },
          riderName: "$userInfo.fullName",
          riderEmail: "$userInfo.email",
          ...
          riderStatus: "$riderInfo.status",
          riderRating: "$riderInfo.rating",
          activeStatus: "$riderInfo.activeStatus",
          joinedAt: "$riderInfo.createdAt"
        }
      }
    ]);
    ...
    sendSuccess(res, {
      ...
      riders: topRiders,
      generatedAt: new Date()
    }, "Top riders by activity retrieved successfully", 200);
  } catch (err) {
...
/**
 * Get real-time analytics summary (lightweight for WebSocket updates)
 */
exports.getRealTimeAnalytics = async (req, res) => {
  try {
    ...
    sendSuccess(res, realTimeData, "Real-time analytics retrieved successfully", 200);
  } catch (err) {
    console.error("Get real-time analytics error:", err);
    sendError(res, "Failed to retrieve real-time analytics", 500);
  }
};
